...
namespace Pedidos.DAL.Repositorio
{
    public class RepositorioGenerico<TEntidad> : IRepositorioGenerico<TEntidad>
        where TEntidad : class
    {
        #region Variables
        private readonly ComprasContext _comprasContexto;
        private DbSet<TEntidad> _dbEntidad;
        #endregion

        #region Constructor
        public RepositorioGenerico(ComprasContext comprasContexto)
        {
            if (comprasContexto == null)
            {
                comprasContexto = new ComprasContext();
            }
